...
export default async (req: Request) => {
  ...
  try {
    ...
    if (!base64Data || !mimeType) {
      return new Response(
        JSON.stringify({ error: "Faltan los campos base64Data o mimeType" }),
    ...
    if (!responseText) {
      return new Response(
        JSON.stringify({ error: "No response text from Gemini" }),
    ...
    console.error("extract-receipt function error:", errMessage);
    return new Response(
      JSON.stringify({
        error: "Error al extraer datos del comprobante",
        details: errMessage,
      }),
